...
void leftRotate(unsigned char *bits, size_t bytes, size_t n) {
	...
	// bitsblock SEC = shiftl128(block->a, block->b, block->c, block->d, (bytes * 8) - n);

	// bitsblock out = {first.a | SEC.a, first.a | SEC.a, first.a | SEC.a, first.a | SEC.a};
...
void encrypt(/*size_t block*/) {
	...
	size_t numBlocks = realPlainSize / BLOCK_SIZE;
	// unsigned char state[plainSize];
	unsigned char *state = (unsigned char *) malloc(numBlocks * BLOCK_SIZE);
	// reverse block order because chaos
	#pragma omp parallel for
	for (int i = 0; i <  BLOCK_SIZE; ++i) {
		memcpy(state, plain + realPlainSize - i * BLOCK_SIZE, BLOCK_SIZE);
	}

	unsigned char *w = (unsigned char *) malloc(numBlocks * (NUM_ROUNDS + 1) * KEY_SIZE);
	// unsigned char w[numBlocks * (NUM_ROUNDS + 1) * KEY_SIZE];

	clock_t current1 = clock();
	//printf("Timing: Initialize stuff in %f\n", (double) (current1 - start) / CLOCKS_PER_SEC);

	// expand key here (differs from original alg)
	expandKey(key, w);

	clock_t current2 = clock();
	//printf("Timing: Expand key in %f\n", (double) (current2 - current1) / CLOCKS_PER_SEC);

	addRoundKey(state, plainSize, &w[0], numBlocks - 1);

	current1 = clock();
	//printf("Timing: Add Round Key in %f\n", (double) (current1 - current2) / CLOCKS_PER_SEC);
	...
	for (size_t round = 1; round < NUM_ROUNDS; ++round) {
		...
		for (int i = 0; i < numBlocks; ++i) {
			current1 = clock();
			//printf("Timing: Expand key in %f\n", (double) (current1 - current2) / CLOCKS_PER_SEC);
			subBytes(state + i * BLOCK_SIZE, plainSize);
			shiftRows(state + i * BLOCK_SIZE, plainSize);
			mixColumns(state + i * BLOCK_SIZE, plainSize);
			// 4th parameter doesn't matter
			addRoundKey(state + i * BLOCK_SIZE, plainSize, &w[round * numBlocks * KEY_SIZE], (round + 1) * numBlocks - 1);

			current2 = clock();
			//printf("Timing: Round %ld in %f\n", round, (double) (current2 - current1) / CLOCKS_PER_SEC);
		}
	}
	...
	for (int i = 0; i < numBlocks; ++i) {
		subBytes(state + i * BLOCK_SIZE, plainSize);
		shiftRows(state + i * BLOCK_SIZE, plainSize);
		addRoundKey(state + i * BLOCK_SIZE, plainSize, &w[NUM_ROUNDS * numBlocks * KEY_SIZE], (NUM_ROUNDS + 1) * numBlocks - 1);
	}

	memcpy(cipher /*+ block * BLOCK_SIZE*/, state, realPlainSize);
...
int main () {
	...
	size_t remNumBlocks =  8192 / 16; // the assignment wants it that way 8192 / 16; // openssl seems to do it this way
